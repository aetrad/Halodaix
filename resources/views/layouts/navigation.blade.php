<nav x-data="{ open: false }" class="bg-white border-b border-gray-100">
        ...
        <div class="flex justify-between h-16">
            <div class="flex">
                ...
                <div class="hidden space-x-8 sm:-my-px sm:ml-10 sm:flex">
                    ...
                    <!-- Liens à ajouter -->
                    <x-nav-link :href="route('admin.spartans.index')" :active="request()->routeIs('admin.spartans.*')">
                        {{ __('Spartans') }}
                    </x-nav-link>
                    <x-nav-link :href="route('admin.types.index')" :active="request()->routeIs('admin.types.*')">
                        {{ __('Types') }}
                    </x-nav-link>
                    <x-nav-link :href="route('admin.attacks.index')" :active="request()->routeIs('admin.attacks.*')">
                        {{ __('Attacks') }}
                    </x-nav-link>
                </div>
            </div>
    ...
    <div :class="{ 'block': open, 'hidden': !open }" class="hidden sm:hidden">
        <div class="pt-2 pb-3 space-y-1">
            ...
            <!-- Liens à ajouter -->
            <x-responsive-nav-link :href="route('admin.spartans.index')" :active="request()->routeIs('admin.spartans.*')">
                {{ __('Spartans') }}
            </x-responsive-nav-link>
            <x-responsive-nav-link :href="route('admin.types.index')" :active="request()->routeIs('admin.types.*')">
                {{ __('Types') }}
            </x-responsive-nav-link>
            <x-responsive-nav-link :href="route('admin.attacks.index')" :active="request()->routeIs('admin.attacks.*')">
                {{ __('Attacks') }}
            </x-responsive-nav-link>
        </div>
